...
router.get('/', async (req, res) => {
  ...
  try{
    ...
    return res.status(500).json({ message: 'Error fetching total count', data: null });
  }
...
// Route to retrieve user's optimization history
app.get('/:id/history', async (req, res) => {
  try {
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ message: 'User not found' });

    res.status(200).json({ history: user.optimizationHistory });
  } catch (err) {
    console.error(err);
